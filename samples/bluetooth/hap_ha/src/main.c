...
static void stream_started_cb(struct bt_bap_stream *bap_stream)
{
	...
	__ASSERT(err == 0, "Failed tp get ISO chan info: %d", err);
...
void bap_thread(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		...
		case BAP_STATE_CREATE_SINK:
			err = bt_bap_broadcast_sink_create(pa_sync,
			broadcaster_broadcast_id, &broadcast_sink);
			if (err != 0) {
		...
		case BAP_STATE_WAIT_BIS_REQ:
			...
			sync_bitfield = select_bis_sync_bitfield(
				&base_recv_data, requested_bis_sync);
			if (sync_bitfield == 0U) {
