 ...
 *  Copyright (c) 2024-2026 Demant A/S
 *
...
static uint32_t broadcaster_id;
static volatile bool is_big_synced;
static volatile bool is_base_received;
static struct bt_conn *broadcast_assistant_conn;
static struct bt_bap_stream *bap_streams[CONFIG_BT_BAP_BROADCAST_SNK_STREAM_COUNT];

static bool broadcast_assistant_is_connected(void)
{
	return broadcast_assistant_conn != NULL;
}
			 ...
			 size_t base_size)
{
	...
	if (!broadcast_assistant_is_connected()) {
			   ...
			   const uint32_t bis_sync_req[CONFIG_BT_BAP_BASS_MAX_SUBGROUPS])
{
	...
	LOG_INF("BIS sync req for %p, broadcast id: 0x%06x, (%s)\n", recv_state,
		recv_state->broadcast_id, is_big_synced ? "BIG synced" : "BIG not synced");
	...
	if (is_big_synced) {
		 ...
		 * controller. Thus, when this returns, the `is_big_synced`
		 * is back to false.
		 */
	...
	broadcaster_id = recv_state->broadcast_id;
	if (sync_req) {
		k_sem_give(&sem_bis_sync_requested);
	}
...
/* Connection callbacks for Broadcast Sink */
static void broadcast_snk_connected(struct bt_conn *conn, uint8_t err)
{
	if (err != 0) {
		LOG_WRN("Broadcast assistant connection failed (err 0x%02X)\n", err);
		return;
	}

	if (broadcast_assistant_conn == conn) {
		return;
	}

	if (broadcast_assistant_conn != NULL) {
		LOG_WRN("Ignoring additional assistant connection\n");
		return;
	}

	LOG_INF("Broadcast assistant connected, signaling broadcast sink\n");
	broadcast_assistant_conn = bt_conn_ref(conn);
	k_sem_give(&sem_connected);
}

static void broadcast_snk_disconnected(struct bt_conn *conn, uint8_t reason)
{
	if (conn != broadcast_assistant_conn) {
		return;
	}

	LOG_WRN("Broadcast assistant disconnected (reason 0x%02X)\n", reason);
	bt_conn_unref(broadcast_assistant_conn);
	broadcast_assistant_conn = NULL;
}

BT_CONN_CB_DEFINE(broadcast_snk_callbacks) = {
	.connected = broadcast_snk_connected,
	.disconnected = broadcast_snk_disconnected,
};

static int bap_broadcast_snk_reset(void)
{
	int err;

	req_recv_state = NULL;
	is_big_synced = false;
	is_base_received = false;

	(void)memset(&base_recv_data, 0, sizeof(base_recv_data));
	(void)memset(&requested_bis_sync, 0, sizeof(requested_bis_sync));
	(void)memset(sink_broadcast_code, 0, sizeof(sink_broadcast_code));

	broadcaster_id = BT_BAP_INVALID_BROADCAST_ID;
...
struct broadcast_snk_fsm_ctx {
	uint8_t stream_count;
	uint32_t sync_bitfield;
};

static enum broadcast_snk_state broadcast_snk_state_reset(void)
{
	int err;

	LOG_INF("BROADCAST_SNK_STATE_RESET\n");
	err = bap_broadcast_snk_reset();

	if (err != 0) {
		LOG_WRN("BAP Sink RESET STATE: Reset failed (%d)\n", err);
		k_sleep(K_SECONDS(1));

		return BROADCAST_SNK_STATE_RESET;
	}

	return BROADCAST_SNK_STATE_WAIT_BA;
}

static enum broadcast_snk_state broadcast_snk_state_wait_ba(void)
{
	int err;

	LOG_INF("BROADCAST_SNK_STATE_WAIT_BA\n");
	if (!broadcast_assistant_is_connected()) {
		LOG_INF("Waiting for broadcast assistant to connect\n");
	...
	LOG_INF("Broadcast assistant connected\n");
	...
	return BROADCAST_SNK_STATE_PA_SYNC;
}

static enum broadcast_snk_state broadcast_snk_state_pa_sync(void)
{
	int err;
	...
	return BROADCAST_SNK_STATE_CREATE_SINK;
}

static enum broadcast_snk_state broadcast_snk_state_create_sink(void)
{
	int err;

	LOG_INF("BROADCAST_SNK_STATE_CREATE_SINK\n");
	err = bt_bap_broadcast_sink_create(pa_sync, broadcaster_id, &broadcast_sink);
	if (err != 0) {
	...
	return BROADCAST_SNK_STATE_WAIT_BASE;
}

static enum broadcast_snk_state broadcast_snk_state_wait_base(void)
{
	int err;
	...
	return BROADCAST_SNK_STATE_WAIT_SYNCABLE;
}

static enum broadcast_snk_state broadcast_snk_state_wait_syncable(void)
{
	int err;
	...
	return BROADCAST_SNK_STATE_WAIT_CODE;
}

static enum broadcast_snk_state broadcast_snk_state_wait_code(void)
{
	int err;
	...
	return BROADCAST_SNK_STATE_WAIT_BIS_REQ;
}

static enum broadcast_snk_state broadcast_snk_state_wait_bis_req(struct broadcast_snk_fsm_ctx *ctx)
{
	int err;
	...
	ctx->sync_bitfield = select_bis_sync_bitfield(&base_recv_data, requested_bis_sync);
	if (ctx->sync_bitfield == 0U) {
		LOG_WRN("No valid BIS sync found, resetting\n");

		return BROADCAST_SNK_STATE_RESET;
	}

	ctx->stream_count = get_stream_count(ctx->sync_bitfield);
	LOG_INF("Syncing to broadcast with bitfield: 0x%08x, stream_count = %u\n",
		ctx->sync_bitfield, ctx->stream_count);

	return BROADCAST_SNK_STATE_SYNC_BIS;
}

static enum broadcast_snk_state broadcast_snk_state_sync_bis(struct broadcast_snk_fsm_ctx *ctx)
{
	int err;

	err = bt_bap_broadcast_sink_sync(broadcast_sink, ctx->sync_bitfield, bap_streams,
					 sink_broadcast_code);
	if (err != 0) {
	...
	return BROADCAST_SNK_STATE_WAIT_STREAM;
}

static enum broadcast_snk_state broadcast_snk_state_wait_stream(void)
{
	int err;
	...
	return BROADCAST_SNK_STATE_WAIT_DISCONNECT;
}

static enum broadcast_snk_state broadcast_snk_state_wait_disconnect(void)
{
	(void)k_sem_take(&sem_pa_sync_lost, K_FOREVER);

	return BROADCAST_SNK_STATE_WAIT_STOP;
}

static enum broadcast_snk_state broadcast_snk_state_wait_stop(void)
{
	int err;
...
{
	enum broadcast_snk_state state = BROADCAST_SNK_STATE_RESET;
	struct broadcast_snk_fsm_ctx ctx = {0};
	...
	while (true) {
		...
		case BROADCAST_SNK_STATE_WAIT_BIS_REQ:
			state = broadcast_snk_state_wait_bis_req(&ctx);
			break;

		case BROADCAST_SNK_STATE_SYNC_BIS:
			state = broadcast_snk_state_sync_bis(&ctx);
			break;
...
void bap_broadcast_snk_start_thread(void)
{
	k_thread_create(&snk_tid, snk_thread_stack, K_THREAD_STACK_SIZEOF(snk_thread_stack),
			bap_broadcast_snk_thread, NULL, NULL, NULL, THREAD_PRIORITY, 0, K_NO_WAIT);
}
...
int bap_broadcast_snk_init(void)
{
	...
	stream_rx_get_streams(bap_streams);
